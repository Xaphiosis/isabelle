...
import java.awt.event.{MouseMotionAdapter, MouseAdapter, MouseEvent,
  FocusAdapter, FocusEvent, WindowEvent, WindowAdapter, KeyEvent, InputEvent}
...
import org.gjt.sp.jedit.{View, Registers}
  ...
  private val mouse_motion_listener = new MouseMotionAdapter {
    override def mouseDragged(evt: MouseEvent): Unit = {
      robust_body(()) {
        ...
        if (sel != null && (evt.getModifiersEx() & InputEvent.BUTTON1_DOWN_MASK) != 0) {
